...
// Client talks to the GitHub API with an App JWT. It never logs or returns
// credential material in errors beyond the safe GitHub message field.
type Client struct {
	BaseURL    string
	HTTPClient *http.Client
}

// NewClient builds a client for api.github.com that does not follow
// redirects (which could leak the Authorization header).
...
// FindInstallation discovers the App installation ID for a repository.
func (c *Client) FindInstallation(ctx context.Context, appJWT, repository string) (int64, error) {
	...
	if err := c.request(ctx, http.MethodGet, path, appJWT, nil, &response); err != nil {
...
// CreateInstallationToken mints an installation token, repository-scoped when
// repository names are supplied.
func (c *Client) CreateInstallationToken(ctx context.Context, appJWT string, installationID int64, repositories ...string) (string, time.Time, error) {
	...
	if err := c.request(ctx, http.MethodPost, path, appJWT, body, &response); err != nil {
...
func (c *Client) request(ctx context.Context, method, path, appJWT string, body []byte, destination any) error {
...
// SplitRepository validates owner/name.
...
// providerError formats broker-safe errors: messages never include tokens,
// JWTs, or private-key bytes.
type providerError string

func (err providerError) Error() string { return string(err) }

func errorf(format string, args ...any) error { return providerError(fmt.Sprintf(format, args...)) }
